...
این الگوریتم با ذخیره کردن timestamp هر درخواست کار می‌کند.
وقتی یک درخواست جدید دریافت می‌کنیم:
- ابتدا تمام timestamp های قدیمی شده رو پاک می‌کنیم.
- درخواست جدید را رکورد می‌کنیم
- در نهایت با چک کردن اندازه رکورد هامون، تصمیم میگیریم که requst رو بلاک کنیم یا نه.
...
بعد درخواست فعلی رو رکورد اضافه می‌کنیم. چون اندازه لاگمون بیشتر از ۵ نیست،
...
بازه ها توی
fixed window
ثابت اما اینجا پویا اند در نتیجه این الگوریتم گارانتی می‌کند که درخواست هایی فقط
قبول می‌شوند که تعداد درخواست های
از t ثانیه قبلشون تا به الان
از حد قانون یا rate
بیشتر نباشد.

اما در fixed window این گارانتی وجود ندارد در نتیجه اگه قانونی مثل
...
چون ما باید لیستی از لاگ ها برای هر کاربر ذخیره کنیم، دقیقا نمی‌دونیم که هر کاربر چقدر مموری نیاز داره
